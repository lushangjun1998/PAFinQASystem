...
class HybridFieldRetriever:
    ...
    def __init__(self):
        ...
        for field_info in knowledge['fields']:
            doc = Document(
                page_content=f"{field_info['table']} {field_info['field']} {' '.join(field_info['synonyms'])}",
    ...
    def retrieve_fields(self, question: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
        检索相关字段和表
        返回: (字段列表, 表列表)
        """
    ...
    def get_relevant_context(self, question: str) -> str:
        """获取检索到的上下文信息，用于LLM提示"""
        fields, tables = self.retrieve_fields(question)
